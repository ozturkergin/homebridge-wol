  ...
  async wake(): Promise<void> {
    ...
    if (this.config.startCommand) {
      ...
      try {
        ...
        this.log.error(
          "HWOL-4001 An error occurred while trying to start the device. This is most likely not an issue with homebridge-wol-ergin itself."
        );
        this.log.debug(error);
      }
    ...
    if (woke && this.config.wakeCommand) {
      ...
      try {
        ...
        this.log.error(
          "HWOL-4001 An error occurred while trying to wake the device. This is most likely not an issue with homebridge-wol-ergin itself."
        );
        this.log.debug(error);
      }
    }
  ...
  async shutdown(): Promise<void> {
    ...
    if (this.config.shutdownCommand) {
      ...
      try {
        ...
        this.log.error(
          "HWOL-4001 An error occurred while trying to shut down the device. This is most likely not an issue with homebridge-wol-ergin itself."
        );
        this.log.debug(error);
      }
